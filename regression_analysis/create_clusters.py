...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset-path', type=str, required=True, help='Dataset to create clusters from')
    parser.add_argument('--embeddings-path', type=str, required=True, help='Embeddings to create clusters from')
    parser.add_argument('--visualization-save-dir', type=str, required=True, help='Directory to save clusters to')
    parser.add_argument('--clusters-save-dir', type=str, required=True, help='Directory to save clusters to')
    # use this only to test, no cache to generate clusters to increase diversity
    parser.add_argument('--use-cache', action='store_true', help='Use cache')
    # clustering parameters
    parser.add_argument('--umap-n-neighbors', type=int, default=5, help='Number of neighbors for UMAP')
    parser.add_argument('--umap-min-dist', type=float, default=0.0001, help='Minimum distance for UMAP')
    parser.add_argument('--clustering-method', type=str, default='dbscan', choices=['dbscan', 'hdbscan', 'kmeans', 'gmm'], help='Clustering method')
    parser.add_argument('--dbscan-eps', type=float, default=0.2, help='Epsilon for DBSCAN')
    parser.add_argument('--dbscan-min-samples', type=int, default=100, help='Minimum samples for DBSCAN')
    parser.add_argument('--hdbscan-min-cluster-size', type=int, default=4000, help='Minimum cluster size for HDBSCAN')
    parser.add_argument('--hdbscan-min-samples', type=int, default=10, help='Minimum samples for HDBSCAN')
    parser.add_argument('--hdbscan-cluster-selection-method', type=str, default='leaf', help='Cluster selection method for HDBSCAN')
    parser.add_argument('--hdbscan-cluster-selection-epsilon', type=float, default=0.0)
    parser.add_argument('--kmeans-n-clusters', type=int, default=5)
    parser.add_argument('--gmm-n-components', type=int, default=5)
    parser.add_argument('--plot-focus-umaps', action='store_true')
    parser.add_argument('--no-noise', action='store_true')
    parser.add_argument('--min-focus-cluster-size', type=int, default=80-000)
    ...
    assert len(full_dataset_embeddings) == len(dataset)


    cache_dir = Path(args.clusters_save_dir) / "cache"

    os.makedirs(args.clusters_save_dir, exist_ok=True)
    os.makedirs(args.visualization_save_dir, exist_ok=True)
    os.makedirs(cache_dir, exist_ok=True)

    print(f"Cache dir: {cache_dir}")
    print(f"Save dir: {args.clusters_save_dir}")
    print(f"Visualization dir: {args.visualization_save_dir}")
    ...
    all_colors = list(mcolors.TABLEAU_COLORS.keys()) + list(mcolors.XKCD_COLORS.keys()) * 10 # remove white colors
    ...
    plot_and_save(
        ...
        title=f"Sample in the sample UMAP",
        save_path=f"{args.visualization_save_dir}/sample_in_sample_umap/umap_({umap_n_neighbors},{umap_min_dist})/{clustering_string}.png",
    ...
    plot_and_save(
        ...
        save_path=f"{args.visualization_save_dir}/full_dataset_in_sample_umap/umap_({umap_n_neighbors},{umap_min_dist})/{clustering_string}.png",
        legend_label_color_pairs=legend_label_color_pairs
    )
    ...
    plot_and_save(
        ...
        save_path=f"{args.visualization_save_dir}/full_dataset_selected_in_sample_umap/umap_({umap_n_neighbors},{umap_min_dist})/{clustering_string}.png",
        legend_label_color_pairs=legend_label_color_pairs
    )
    ...
    for c_i, (cluster_label, cluster_size) in enumerate(selected_clusters):
        ...
        if args.plot_focus_umaps:
            ...
            plot_and_save(
                ...
                save_path=f"{args.visualization_save_dir}/cluster_in_focus_umap/umap_({umap_n_neighbors},{umap_min_dist})/{clustering_string}/cluster_{cluster_label}.png",
            )
        ...
        cluster_save_path = f"{args.clusters_save_dir}/umap_({umap_n_neighbors},{umap_min_dist})_{clustering_string}_cluster_{cluster_label}"
        per_cluster_results["clusters"][cluster_label] = {
            "cluster_path": cluster_save_path,
            "cluster_indices": full_cluster_indices
        }
        cluster_dataset.save_to_disk(cluster_save_path)

    # save per cluster results
    save_path = f"{args.clusters_save_dir}/results/umap_({umap_n_neighbors},{umap_min_dist})_{clustering_string}.pkl"
    os.makedirs(os.path.dirname(save_path), exist_ok=True)
